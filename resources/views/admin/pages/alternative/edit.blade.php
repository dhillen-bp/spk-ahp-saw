...
@section('content')
    <div class="my-10">
        <h1 class="my-4 text-3xl font-bold">EDIT ALTERNATIVE</h1>
    ...
    <div class="grid grid-cols-1 space-y-5 md:grid-cols-5 md:space-x-10 md:space-y-0">
        ...
        <form method="POST" action="{{ route('admin.alternative.update', $alternative->id) }}"
            ...
            <div class="mb-5">
                ...
                <input type="number" id="no_kk" name="no_kk" required
                    ...
                    value="{{ old('no_kk') ?? $alternative->no_kk }}" />
                @error('no_kk')
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-5">
                ...
                <input type="text" id="alamat" name="alamat" required
                    ...
                    value="{{ old('alamat') ?? $alternative->alamat }}" />
                @error('alamat')
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                @enderror
            </div>
            <div class="mb-5">
                ...
                <select id="jenis_kelamin" name="jenis_kelamin"
                    ...
                    <option value="L"
                        {{ old('jenis_kelamin', $alternative->jenis_kelamin) == 'L' ? 'selected' : '' }}>
                        Laki-Laki</option>
                    <option value="P"
                        {{ old('jenis_kelamin', $alternative->jenis_kelamin) == 'P' ? 'selected' : '' }}>
                        Perempuan</option>
                </select>
                @error('jenis_kelamin')
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
        ...
        <div class="col-span-2 rounded-lg bg-blue-50 p-4">
            <h3 class="text-center text-lg font-bold">Keterangan Form</h3>
