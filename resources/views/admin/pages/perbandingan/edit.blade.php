...
@section('content')
    ...
    <div class="grid grid-cols-1 space-y-5 md:grid-cols-5 md:space-x-10 md:space-y-0">
        ...
        <form method="POST" action="{{ route('admin.perbandingan.update', $kriteriaSelected->id) }}"
            ...
            <div class="mb-5">
                ...
                <input type="number" id="nama" name="nama"
                    ...
                    value="{{ old('nama') ?? $kriteriaSelected->nama }}" />
                @error('nama')
            ...
            <div class="mb-5">
                ...
                <input type="number" id="jumlah_penerima" name="jumlah_penerima"
                    ...
                    value="{{ old('jumlah_penerima') ?? $kriteriaSelected->jumlah_penerima }}" />
            ...
            <div class="mb-5">
                ...
                <textarea id="keterangan" rows="4" name="keterangan"
                    class="{{ $errors->has('keterangan') ? 'input-error' : 'input-default' }} block w-full rounded-lg border p-2.5 text-sm">{{ old('keterangan') ?? $kriteriaSelected->keterangan }}</textarea>
                @error('keterangan')
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-5">
                ...
                <ul class="w-full rounded-lg border border-gray-200 bg-white text-sm font-medium text-gray-900">
                    @php
                        $selectedCriteriaIds = $kriteriaSelected->criteriaComparisons
                            ->pluck('kriteria1_id')
                            ->merge($kriteriaSelected->criteriaComparisons->pluck('kriteria2_id'))
                            ->unique()
                            ->toArray();
                    @endphp
                        ...
                        <li class="w-full rounded-t-lg border-b border-gray-200 dark:border-gray-600">
                            <div class="flex items-center ps-3">
                                <input id="{{ $data->id }}" type="checkbox" value="{{ $data->id }}"
                                    ...
                                    {{ in_array($data->id, $selectedCriteriaIds) ? 'checked' : '' }}>
                                <label for="{{ $data->id }}"
