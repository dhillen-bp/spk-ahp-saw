...
@section('content')
    {{-- BREADCRUMB --}}
    <div class="my-10">
        <h1 class="my-4 text-3xl font-bold">PERBANDINGAN SUBKRITERIA
            {{ Str::upper($subCriteriaComparison[0]->criteria->nama) }}</h1>
            ...
            <ol class="inline-flex items-center space-x-1 rtl:space-x-reverse md:space-x-2">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('admin.kriteria.show', $criteriaId) }}"
    ...
    <div class="grid grid-cols-1 space-y-5 md:grid-cols-5 md:space-x-10 md:space-y-0">
        <div class="md:col-span-3">
            <form action="{{ route('admin.kriteria.sub.compare_update', $criteriaId) }}" method="POST">
                @csrf
                @method('PATCH')

                @foreach ($subCriteriaComparison as $data)
                    @php
                        $nilaiSubcriteria1 = $data->nilai_sub_criteria1;

                        if ($nilaiSubcriteria1 == 9.0) {
                            $value = -7;
                        } elseif ($nilaiSubcriteria1 == 8.0) {
                            $value = -6;
                        } elseif ($nilaiSubcriteria1 == 7.0) {
                            $value = -5;
                        } elseif ($nilaiSubcriteria1 == 6.0) {
                            $value = -4;
                        } elseif ($nilaiSubcriteria1 == 5.0) {
                            $value = -3;
                        } elseif ($nilaiSubcriteria1 == 4.0) {
                            $value = -2;
                        } elseif ($nilaiSubcriteria1 == 3.0) {
                            $value = -1;
                        } elseif ($nilaiSubcriteria1 == 2.0) {
                            $value = 0;
                        } elseif ($nilaiSubcriteria1 == 1.0) {
                            $value = 1;
                        } elseif ($nilaiSubcriteria1 == 0.5) {
                            $value = 2;
                        } elseif ($nilaiSubcriteria1 == 0.333) {
                            $value = 3;
                        } elseif ($nilaiSubcriteria1 == 0.25) {
                            $value = 4;
                        } elseif ($nilaiSubcriteria1 == 0.2) {
                            $value = 5;
                        } elseif ($nilaiSubcriteria1 == 0.167) {
                            $value = 6;
                        } elseif ($nilaiSubcriteria1 == 0.143) {
                            $value = 7;
                        } elseif ($nilaiSubcriteria1 == 0.125) {
                            $value = 8;
                        } elseif ($nilaiSubcriteria1 == 0.111) {
                            $value = 9;
                        } else {
                            $value = '';
                        }
                    ...
                    <div class="mb-10 rounded-lg bg-blue-50 p-5 shadow-lg">
                        <div class="flex justify-between">
                            <p class="font-bold">{{ $data['sub_criteria1']->nama }} </p>
                            <p class="font-bold">{{ $data['sub_criteria2']->nama }}</p>
                        </div>
                        <div class="relative mb-6">
                            <label for="labels-range-input" class="sr-only">Labels range</label>
                            {{-- {{ 'Nilai Value: ' . $value }}
                            {{ 'Nilai Kriteria1: ' . $data->nilai_sub_criteria1 }} --}}
            ...
            @if ($subCriteriaComparison->isNotEmpty())
                <a href="{{ route('admin.kriteria.sub.compareCalculate', $criteriaId) }}">
                    <button
