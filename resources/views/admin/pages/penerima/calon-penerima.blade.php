        ...
        <div class="relative overflow-x-auto sm:rounded-lg">
            <div class="flex-column flex flex-wrap items-center justify-between space-y-4 pb-4 sm:flex-row sm:space-y-0">
                ...
                <div class="flex justify-between gap-3">
                    ...
                    <a href="{{ route('admin.penerima.exportExcelCalon') }}"
                        class="hover:text-primary-700 flex flex-shrink-0 items-center justify-center rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-900 hover:bg-gray-100 focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-200 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white dark:focus:ring-gray-700">
                    ...
                    <tbody class="text-center">
                        @foreach ($rankingResults as $data)
                            <tr
                                ...
                                <td class="px-6 py-4">
                                    <button data-modal-target="keterangan-modal" data-modal-toggle="keterangan-modal"
                                        data-target="#keterangan-modal" data-id="{{ $data->id }}"
                                        data-is-verified="{{ $data->is_verified ? 'Terverifikasi' : 'Tidak Memenuhi Verifikasi' }}"
                                        data-is-verified-desc="{{ $data->is_verified_desc ?? '-' }}"
                                        data-name="{{ $data->alternative->nama }}"
                            ...
                            @component('admin.layouts.modal_keterangan_penerima_datatables')
                            @endcomponent
    ...
    <script type="module">
        document.addEventListener('DOMContentLoaded', () => {
            document.addEventListener('click', (event) => {
                if (event.target.matches('[data-modal-toggle="keterangan-modal"]')) {
                    const button = event.target;
                    const id = button.getAttribute('data-id');
                    const name = button.getAttribute('data-name');
                    const isVerified = button.getAttribute('data-is-verified');
                    const verifiedDesc = button.getAttribute('data-is-verified-desc');
                    // Ambil data-target tanpa tanda #
                    const targetSelector = button.getAttribute('data-target');
                    const modal = document.querySelector(targetSelector);

                    console.log(verifiedDesc); // Untuk debugging

                    if (modal) {
                        console.log("modal ", name);
                        const dataNama = modal.querySelector('.dataNama');
                        const dataIsVerified = modal.querySelector('.dataIsVerified');
                        const dataIsVerifiedDesc = modal.querySelector('.dataIsVerifiedDesc');

                        // Menggunakan innerHTML untuk memasukkan HTML
                        dataNama.innerHTML = `${name}`;
                        dataIsVerified.innerHTML = `${isVerified}`;
                        dataIsVerifiedDesc.innerHTML = `${verifiedDesc}`;
